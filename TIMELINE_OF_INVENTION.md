...
### 2026-04-27 — JIT SSH CA certs, ansible-vault import, recipient rekey, pull-mode daemon

**What:** Four major Ansible-parity gaps closed in one session.

(1) **SSH CA certs for JIT connections** in runsible-connection. New `ssh_cert.rs` module exposes `CaConfig` + `mint_jit_cert(user_pubkey, ca)` which shells `ssh-keygen -s <ca_key> -I <key_id> -n <principal> -V +<seconds>s <pubkey>`. `SshSystemConnection` gained `ca_config: Option<CaConfig>` and now mints a fresh cert before every exec/put_file/get_file when configured (OpenSSH auto-presents the sibling `<id>-cert.pub` alongside `-i <id>`). `ConnectionSpec` gained `ca_key_path` + `ca_principal` + `ca_validity_seconds` so TOML inventories can declare CA-signed JIT auth per host. End-to-end test mints a real Ed25519 CA + user key via ssh-keygen, signs a 120s cert, parses it via `ssh-keygen -L`, verifies principal + key_id round-trip.

(2) **runsible-vault import-ansible** for legacy `$ANSIBLE_VAULT;1.1`/`;1.2` files. New `ansible_import.rs` module: parses the outer-hex-of-(salt:hmac:ciphertext) wire format, derives 80 bytes via PBKDF2-HMAC-SHA256 (10,000 iters), splits to AES-256 key + HMAC-SHA256 key + IV, verifies the MAC (constant-time), decrypts the AES-256-CTR ciphertext, strips PKCS7 padding. CLI subcommand `runsible-vault import-ansible <file> [--password-file PATH | --password STR] [-r ...] [-o OUT]` reads the legacy password (file > flag > stdin), decrypts, and re-encrypts under runsible recipients via the existing `crypto::encrypt_bytes_to_keys` path. 7 unit tests cover the round-trip including the version 1.2 label form, wrong-password HMAC failure, and invalid-header rejection.

(3) **Recipient rekey** in runsible-vault. New `recipients.rs` exposes `rekey_to(envelope, new_recipients, keystore)` as the load-bearing primitive. `add_recipient(envelope, new, existing, keystore)` and `remove_recipient(envelope, drop, existing, keystore)` are thin wrappers. CLI: `recipients add --recipient <new> --existing <e1> --existing <e2>...`, `recipients remove --recipient <drop> --existing <e1>...`, `recipients rekey --recipient <r1> --recipient <r2>...`. Honest about the M1 limitation: age doesn't expose stanza splicing, so rekey re-encrypts the body under a fresh DEK rather than preserving bit-identical ciphertext. The existing-recipients flag is mandatory because age's wrapped-DEK header doesn't reveal the original recipient public keys (we'd silently drop unknown recipients otherwise). 9 tests cover add/remove/rekey/list with revocation verification (dropped recipient can't decrypt the new file).

(4) **runsible-pull daemon mode**. New `daemon.rs` (loop-with-jitter) and `http_heartbeat.rs` (POST via system `curl` with retries, exponential backoff, on-failure NDJSON queue, drain-on-success). `PullConfig` gained `[schedule]` (interval + jitter) and `[heartbeat]` (url + bearer_token + timeout + max_retries + initial_backoff_seconds + queue_path) sections. CLI: `runsible-pull --daemon --config <path> [--interval <duration>]`. Duration parser accepts `s/m/h/d` suffixes. Jitter uses a deterministic LCG keyed on (cycle_idx, pid) so tests are reproducible without a rand crate. 7 daemon + http_heartbeat tests cover empty-url-no-op, NDJSON enqueue+drain, unreachable-URL retries-then-queues, jitter bounds, run-then-stop semantics.

Workspace: 553/553 tests pass (was 522 before this session; +31 new). 14/14 TRIPLE SIMS gates still green.
...
### 2026-04-27 — Test coverage pass: 354 unit tests + 14 exopack TRIPLE SIMS gates

**What:** Two-stage push on test rigor. Stage one: 6 parallel agents added 200+ unit tests across the workspace (148 → 354). Per-crate increments: runsible-core 0→12, runsible-config 4→15, runsible-inventory 6→29, runsible-vault 8→20, runsible-connection 10→29, yaml2toml 7→21, runsible-playbook 52→113, runsible-galaxy 10→20, runsible-lint 10→20 (every L001–L020 individually), runsible-doc 10→20, runsible-pull 12→20, runsible-test 7→15, runsible-console 7→15. Surfacing one source bug along the way: `runsible-inventory::parse_inventory` was silently accepting dangling `children = ["nonexistent"]` references — the new test forced raising `InventoryError::UnknownChild` with a real validation pass. Stage two: added `pub fn f30() -> i32` to every crate (14 of them) plus a `<crate>-test` binary that calls `exopack::triple_sims::f60` to run f30 three times — all three must pass. Each f30 exercises a real public-API path end-to-end (encrypt+decrypt round-trip for vault, parse+pattern-match+range-expand for inventory, set_fact→template→debug for playbook, etc.) and returns distinct non-zero codes per failure stage so triple-sims output identifies which step regressed. No self-licking-ice-cream-cone tests: every f30 constructs real input → runs real code paths → checks real output, no `assert_eq!(constant, constant)`.
**Why:** "148 is laughable" — the user's directive. Test count was light for a 14-crate workspace targeting Ansible parity. Adding f30 + TRIPLE SIMS per crate gives a single command per crate (`./target/debug/<crate>-test`) that gates the whole crate with a deterministic 3-pass check, which is what the cochranblock provenance framework requires for federal acquisition compliance.
**Commit:** bceb2e7 (unit-test expansion), 3a46638 (exopack gates)
**AI Role:** AI ran 6 parallel agents for the unit-test expansion, then 3 parallel agents for the exopack wiring (split: foundation/adapter, engine/galaxy, surface). Human directed the rigor bar ("no self-licking ice cream cones"), the framework choice (exopack TRIPLE SIMS per cochranblock provenance-docs convention), and the f30 entrypoint pattern.
**Proof:** `~/.cargo/bin/cargo test --workspace` — 354 tests pass; loop running `./target/debug/<crate>-test` for all 14 crates — 14/14 print "TRIPLE SIMS: 3/3 passes OK" and exit 0.
